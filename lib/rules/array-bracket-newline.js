...
exports.default = utils_1.ESLintUtils.RuleCreator.withoutDocs({
    ...
    create(context) {
        ...
        function reportNoBeginningLinebreak(node, token) {
            context.report({
                ...
                fix(fixer) {
                    ...
                    if (!nextToken || utils_1.ASTUtils.isCommentToken(nextToken) || utils_1.ASTUtils.isOpeningBraceToken(nextToken)) {
                        return null;
                    }
        ...
        function reportNoEndingLinebreak(node, token) {
            context.report({
                ...
                fix(fixer) {
                    ...
                    if (!previousToken || utils_1.ASTUtils.isCommentToken(previousToken) || utils_1.ASTUtils.isClosingBraceToken(previousToken)) {
                        return null;
                    }
        ...
        function check(node) {
             ...
             *     ]
             */
            if (utils_1.ASTUtils.isOpeningBraceToken(first)) {
                const next = sourceCode.getTokenAfter(first);
                const before = sourceCode.getTokenBefore(last);
                console.log('what is', next);
                if (utils_1.ASTUtils.isTokenOnSameLine(openBracket, next)) {
                    reportRequiredBeginningLinebreak(node, first);
                }
                if (utils_1.ASTUtils.isTokenOnSameLine(node, before)) {
                    reportRequiredEndingLinebreak(node, last);
                }
            }
            else if (needsLinebreaks) {
                if (utils_1.ASTUtils.isTokenOnSameLine(openBracket, first)) {
                    reportRequiredBeginningLinebreak(node, openBracket);
                }
                if (utils_1.ASTUtils.isTokenOnSameLine(last, closeBracket)) {
